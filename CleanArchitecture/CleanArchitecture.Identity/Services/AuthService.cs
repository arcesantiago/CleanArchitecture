...
namespace CleanArchitecture.Identity.Services
{
    public class AuthService : IAuthService
    {
        ...
        public async Task<AuthResponse> Login(AuthRequest request)
        {
            ...
            if(user == null) throw new Exception($"El usuario con el email {request.Email} no existe");

            var resultado = await _signInManager.PasswordSignInAsync(user.UserName,request.Password,false,lockoutOnFailure: false);

            if (!resultado.Succeeded) throw new Exception("Las credenciales son incorrectas");
